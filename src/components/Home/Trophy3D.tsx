...
function TrophyModel({ modelPath }: { modelPath: string }) {
    ...
    React.useEffect(() => {
        ...
        // Scale to make trophy height approximately 3.5-4 units (fills ~40-45vh)
...
function PlaceholderTrophy() {
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      meshRef.current.rotation.y += delta * 0.5; // Adjust speed here (0.5 = moderate speed)
    }
  });
...
const Trophy3D = ({ modelPath }: { modelPath?: string }) => {
  return (
